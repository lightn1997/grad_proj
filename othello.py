...
class OthelloEnv(gym.Env):
    ...
    def step(self, action):
        ...
        while not done and self.env.player_turn != self.protagonist:
            ...
            new_obs, reward, done, _ = self.env.step(opponent_move)
            self.opponent.step(obs, opponent_move, reward, new_obs, done)
            obs = new_obs
            if self.render_in_step:
                self.render()
        return obs, -reward, done, None
...
class OthelloBaseEnv(gym.Env):
    ...
    def __init__(self,
        ...
        self.initial_walls = min(initial_walls, board_size ** 2 - 4)
    ...
    def _reset_board(self):
        ...
        if self.initial_walls:
            all_positions = [(row, col) for row in range(self.board_size) for col in range(self.board_size)]
            excluded_positions = [(center_row_ix - 1, center_col_ix - 1), (center_row_ix, center_col_ix), (center_row_ix, center_col_ix - 1), (center_row_ix - 1, center_col_ix)]
            remaining_positions = [pos for pos in all_positions if pos not in excluded_positions]
            walls = random.sample(remaining_positions, max(self.initial_walls, 0))
            for row, col in walls:
                board_state[row][col] = WALL
        return board_state
            ...
            for j in range(self.board_size):
                ...
                if self.board_state[i][j] == WHITE_DISK:
                    ...
                    disk = make_disk_at(
                        ...
                        radius=half_grid_width - 3)
                    disk.render1()
                elif self.board_state[i][j] != NO_DISK:
                    gl.glColor4f(0.5, 0.5, 0.5, 1.)
                    wall = make_wall_at(
                        x=i * grid_width + half_grid_width,
                        y=j * grid_width + half_grid_width,
                        size=half_grid_width - 1)
                    wall.render1()
